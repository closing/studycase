...
public class AppBoot {
	public static void main(String[] args) throws InterruptedException {
		...
		final Flux<Long> source = Flux.interval(Duration.ofMillis(1000)).take(10).publish().autoConnect();
		source.subscribe();
		Thread.sleep(5000);
		...
		Flux.just(1, 2).concatWith(Mono.error(new IllegalArgumentException())).retry(1).onErrorResume(e -> {
		...
		Flux.just(1, 2).concatWith(Mono.error(new IllegalStateException())).onErrorReturn(0).subscribe(
				System.out::println);
		...
		Flux.range(1, 100).reduceWith(() -> 100, (s1, s2) -> s1 + s2).subscribe(System.out::println);
